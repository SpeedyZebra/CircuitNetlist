...
def visual_drc_from_scene(scene: SchematicScene) -> tuple[list[Diagnostic], dict[str, Any]]:
    ...
    for element in scene.elements:
        ...
        if element.text and element.metadata.get("drc_wire_text", True):
            ...
            text_items.append((element.id, element.parent_id or "", element.net_name or "", element.kind, element.text.bounds.as_tuple()))
    ...
    for net, kind, source_ref, segment, *_ in wire_like:
    ...
    wire_text_overlaps = sum(1 for _, _, _, segment, *_ in wire_like for ref, box in text_boxes if not _segment_touches_component_pin(segment, pin_points.get(ref, set())) and segment_crosses_box(segment, inflate_box(box, 8)))
    physical_wire_text_overlaps = sum(1 for _, kind, _, segment, *_ in wire_like if kind == "wire" for ref, box in physical_text_boxes if not _segment_touches_component_pin(segment, pin_points.get(ref, set())) and segment_crosses_box(segment, inflate_box(box, 8)))
...
def wire_like_segments(circuit: Circuit, library: Any, layout: Layout, routes: Any) -> list[tuple[str, str, str, tuple[int, int, int, int], str, str]]:
    return wire_like_segments_from_scene(build_schematic_scene(circuit, library, layout, list(routes)))


def wire_like_segments_from_scene(scene: SchematicScene) -> list[tuple[str, str, str, tuple[int, int, int, int], str, str]]:
    segments: list[tuple[str, str, str, tuple[int, int, int, int], str, str]] = []
    for element in scene.elements:
